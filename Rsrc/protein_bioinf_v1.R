...
#######################################################
# A SAFE version of ape::read.FASTA
#######################################################
# NOTE: You can have a HORRIFYING bug if you forget to say:
# seqs = ape::read.FASTA(filename, type="AA")

# Solution: no default on "type"
read_FASTA_safe <- function(file, type)
	{
	read_FASTA(file, type)
	}

read_FASTA <- function(file, type)
	{
	ex='
	# Compare:
	# 2025-01-26_ #@)_(*@#_)*( HUGE ISSUE
	
	# Example of how you can introduce a horrifying bug into your code,
	# if you read your amino acid FASTA file with ape::read.FASTA(file), 
	# instead of ape::read.FASTA(file, type="AA"). If not specified, by
	# default it assumes the FASTA file is DNA, then...DROPS ALL CHARACTERS
	# THAT ARE NOT IUPAC-DNA FORMAT, i.e. over half the AA characters are 
	# just lost.
	#
	# Amazingly, BLAST can often still figure out the original source sequence,
	# if the AA sequence was long enough...but I had a painful time until I 
	# figured out the issue.
	#
	# Example graphic showing what happened to the sequence is pasted below.
	#
	# sourceall("/GitHub/str2phy/Rsrc/") 
	#
	# now loads read_FASTA and read_FASTA_safe which require the user to 
	# manually specify the tup.
	...
	# NAD1 mitochondrial AA sequences, from mitoCOGs dataset
	fixfn = "/GitHub/str2phy/Rsrc/tests/read_FASTA_bug/NAD1_35_AAs_fixnames.fasta"
	
	fixseqs = read.FASTA(fixfn)
	tmpjunk = as.character(fixseqs)
	tmpjunk[[1]]
	length(tmpjunk[[1]]) # 169 "AAs" long
	
	fixseqs = read.FASTA(fixfn, type="DNA")
	tmpjunk = as.character(fixseqs)
	tmpjunk[[1]]
	length(tmpjunk[[1]]) # 169 "AAs" long

	fixseqs = read.FASTA(fixfn, type="AA")
	tmpjunk = as.character(fixseqs)
	tmpjunk[[1]]
	length(tmpjunk[[1]]) # 312 AAs is the *ACTUAL* length. 
											 # All non-DNA characters were droped
	
	# Here is a safe fix - gives error message if AAs not specified
	fixseqs = read_FASTA(fixfn)
	fixseqs = read_FASTA_safe(fixfn)

	fixseqs = read_FASTA(fixfn, type="AA")
	tmpjunk = as.character(fixseqs)
	tmpjunk[[1]]
	length(tmpjunk[[1]]) # 312 AAs is the *ACTUAL* length. 
											 # All non-DNA characters were droped
	...
	# Compare to seqinr read.fasta
	fixseqs = seqinr::read.fasta(fixfn)
	fixseqs[[1]]
	as.character(fixseqs[[1]])
	length(as.character(fixseqs[[1]]))
	# class SeqFastadna, lower-case, but doesnt drop characters
	
	fixseqs = seqinr::read.fasta(fixfn, seqtype="DNA")
	fixseqs[[1]]
	as.character(fixseqs[[1]])
	length(as.character(fixseqs[[1]]))
	# class SeqFastadna, lower-case, but doesnt drop characters
	
	fixseqs = seqinr::read.fasta(fixfn, seqtype="AA")
	fixseqs[[1]]
	as.character(fixseqs[[1]])
	length(as.character(fixseqs[[1]]))
	# class SeqFastaAA, lower-case, but doesnt drop characters
	' # END example

	seqs = ape::read.FASTA(file=file, type=type)
	return(seqs)
	} # END read_FASTA
...
classify_MotBfam_labels <- function(list_of_strings)
	{
	junk='
	
	list_of_strings = protein_name
	short_protname = classify_MotBfam_labels(list_of_strings=protein_name)
	short_protname
	'
	...
	for (i in 1:length(list_of_strings))
		...
		tmpstr = gsub(pattern="probable biopolymer transport protein", replacement="biopoly_transp", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="Biopolymer transport proteins", replacement="biopoly_transp", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="ExbD protein", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="related to flagellar apparatus MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="", replacement="", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="", replacement="", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="", replacement="", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="", replacement="", x=tmpstr, ignore.case=TRUE)
		...
		# Many are just MotB/TolR/ExbD
		if (grepl(pattern="MotB/TolR/ExbD", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "AQB"
			short_protname[i] = tmpstr
			next()
		...
		# Remove obvious words - flagellum
		tmpstr = gsub(pattern="transport membrane proton channel, TolR-related protein", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="transport membrane proton channel, TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="transport ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		...
		tmpstr = gsub(pattern="proton channel family", replacement="", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="flagellar motor component", replacement="", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="flagellar stator protein MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="MotB protein", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="motility protein B", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="flagellar basal body stator protein MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="endoflagellar protein", replacement="flag", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="flagellar protein", replacement="flag", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="MAG\\: flagellar stator protein MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="MotB\\; MotB component of the H\\+\\-coupled stator flagellum complex", replacement="MotB", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="Mot family proton \\(H\\+\\) or sodium \\(Na\\+\\) transporter MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="sodium channel stator-force generator subunit of flagellar rotation", replacement="sodium_MotB", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="signal recognition particle-docking protein FtsY", replacement="SRP_FtsY", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="sodium-driven polar flag MotB", replacement="sodiumPolarMotB", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="MotB\\, \\(MotB\\)", replacement="MotB", x=tmpstr, ignore.case=TRUE)
 		tmpstr = gsub(pattern="MAG: flagellar stator protein MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="pomB protein", replacement="pomB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="probable ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="protein of unassigned function", replacement="unk", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="multi-sensor hybrid histidine kinase", replacement="HistKin", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="MotB\\; MotB component of the H\\+\\-coupled stator flagellum complex", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="Mot family proton \\(H+\\) or sodium \\(Na+\\) transporter MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="membrane spanning protein in TonB-ExbD-ExbD complex", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="flagellar stator protein MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		#tmpstr = gsub(pattern="", replacement="", x=tmpstr, ignore.case=TRUE)
		#tmpstr = gsub(pattern="", replacement="", x=tmpstr, ignore.case=TRUE)
...
#			next()
#			}
		if (grepl(pattern="OmpA", x=tmpstr, ignore.case=FALSE) == TRUE)
			{
			tmpstr = "OmpA"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="Fis_family", x=tmpstr, ignore.case=FALSE) == TRUE)
			{
			tmpstr = "Fis"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="Fis family", x=tmpstr, ignore.case=FALSE) == TRUE)
			{
			tmpstr = "Fis"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="TonB2 energy transduction system inner membrane component ExbD", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD_wTonB2"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="TonB1 energy transduction system for heme uptake inner membrane component ExbD", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD_wTonB1"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="TolR-type precursor", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="TolR-transmembrane prediction", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="TolR-related transport transmembrane protein", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="TolR-like", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="TolR transport transmembrane protein", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="TolR colicin import protein", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="TolA energy-transducing system inner membrane component TolR", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="Tol-Pal system TolR", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="tol transport systemTolR", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="ExbD; membrane protein", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="ExbD transport", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="ExbD family protein", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="\\(MotB\\) transmembrane", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="transmembrane protein MotB", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="related to flagellar apparatus \\MotB\\)", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="MotB \\(flagellar rotation\\)", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="lateral B", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "lateral_MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="probable transmembrane protein MotB", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="stator-force generator of H+ coupled flagellar MotB", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="stator-force generator of Na+ coupled flagellar pomB", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "pomB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="sodium MotB", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "pomB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="Polar sodium-driven \\(MotB\\)", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "pomB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="membrane spanning protein in TolA-TolR-TolR complex; proton channel component", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="membrane spanning protein in TolA-TolR-TolR complex", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="inner membrane TolR ", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="tonB system transport protein ExbD/TolR", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR_ExbD_wTonB"
			short_protname[i] = tmpstr
			next()
			}
		...
		if (grepl(pattern="ExbD/TolR", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD_TolR"
			short_protname[i] = tmpstr
			next()
		...
		tmpstr = gsub(pattern="Biopolymer", replacement="", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="domain-containing protein", replacement="", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="colicin uptake protein TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="ExbD-related protein", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="ferric siderophore transport system,", replacement="", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="probable TolR-type", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="probable TolR protein", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="Cell division and transport-associated protein TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolR protein", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="Tol-Pal system protein TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="Protein TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="(ExbD-like)", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="Ton complex subunit ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolR-like protein", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolR transporter", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="Tol-Pal system subunit TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="\\(ExbD\\)", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="ExbD protein", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="tonB\\-system energizer ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TonB ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="probable ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		...
		tmpstr = gsub(pattern="Cell division and transportassociated TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="tonBsystem energizer ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolPal system TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolRtype", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolPal system subunit TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolRlike protein", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolRrelated protein", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="sodium channel statorforce generator subunit of flagellar rotation", replacement="sodium MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="related to TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="related to \\(TolR\\)", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="tolerance to group A colicins, singlestranded DNA filamentous phage, required for OM integrity", replacement="TolA_OM", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="\\(ExbDlike\\)", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="ExbDrelated protein", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="ExbDlike", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="ExbDrelated protein", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="ExbDlike protein", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="Cell division and transportassociated TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="tonBsystem energizer ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolPal system TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="tolpal systemassociated acylCoA thioesterase", replacement="tolpal thioest", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="sodiumdriven polar flag MotB", replacement="sodium MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="MotB MotB component of the H+coupled stator flagellum complex", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="hypothetical proteintransmembrane prediction", replacement="HYP", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="hypothetical protein", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="probable TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="membrane spanning protein in TonBExbDExbD complex", replacement="ExbD", x=tmpstr, ignore.case=TRUE)




		tmpstr = gsub(pattern="MAG: TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="hypothetical membrane protein", replacement="HYP", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="hypothetical protein-transmembrane prediction", replacement="", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern=", TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="tolerance to group A colicins, single-stranded DNA filamentous phage, required for OM integrity", replacement="colicin-tol", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="tol-pal system-associated acyl-CoA thioesterase ", replacement="TolPal_thioester", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="two-component sensor histidine kinase YdfI", replacement="2compSens_hisKin_YdfI", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="protein of unassigned function", replacement="unk", x=tmpstr, ignore.case=TRUE)
		...
		tmpstr = gsub(pattern="flagellar stator protein MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="MAG: MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="flagellar protein", replacement="flag", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="flagellar proton channel", replacement="flag", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="MAG\\: MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		...
		tmpstr = gsub(pattern="TonB ExbD2", replacement="ExbD2", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TonB ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TonB ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="LafT protein", replacement="LafT", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="transporter ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="LafT protein", replacement="LafT", x=tmpstr, ignore.case=TRUE)
		...
		tmpstr = gsub(pattern="MotB_MotB_component_of_the_H\\+coupled_stator_flagellum_complex", replacement="MotB", x=tmpstr)
		tmpstr = gsub(pattern="MotB_MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="Mot_family_proton_H\\+_or_sodium_Na\\+_transporter_MotB", replacement="MotB", x=tmpstr)
		tmpstr = gsub(pattern="related_to_flagellar_apparatus_MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="related_to_ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="transporter_ExbD", replacement="ExbD", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="conserved_TolR", replacement="TolR", x=tmpstr, ignore.case=TRUE)
		tmpstr = gsub(pattern="MotB_MotB", replacement="MotB", x=tmpstr, ignore.case=TRUE)
		
		if (grepl(pattern="Possible_,_ExbD_family", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="MotB:_;_proton_conductor_component_of_,_torque_generator", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="MotB;_membrane_protein;_signal_peptide", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="MotB;_membrane_protein", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="MotB_flagellar_stator_MotB", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="MotB_duplicate", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="MotB_\\(plasmid\\)", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="MotB_(MotB)", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="MotB", x=tmpstr, ignore.case=FALSE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="membrane_spanning_protein_in_TolA-TolR-TolR_complex;_proton_channel_component", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="membrane_spanning_protein_in_TolA-TolR-TolR_complex", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="inner_membrane_TolR", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="Hypothetical_MotB_flagellar_transmembrane_protein", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="ExbD\\TolR_FAMILY", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD_TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="ExbD_type-1", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="ExbD_family_TolR", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD_TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern="\\(MotB\\)", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (grepl(pattern=",_ExbD_family", x=tmpstr, ignore.case=TRUE) == TRUE)
			{
			tmpstr = "ExbD"
			short_protname[i] = tmpstr
			next()
		...
		if (tmpstr == "sodiumPolarMotB")
			{
			tmpstr = "MotB_Na_polar"
			short_protname[i] = tmpstr
			next()
			}
		...
		if (tmpstr == "DUFx/ExbD_domain_protein")
			{
			tmpstr = "ExbD/DUFx"
			short_protname[i] = tmpstr
			next()
		...
		if (tmpstr == "TolR-transmembrane_prediction")
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (tmpstr == "tol_transport_systemTolR")
			{
			tmpstr = "TolR"
			short_protname[i] = tmpstr
			next()
		...
		if (tmpstr == "stator-force_generator_of_Na+_coupled_flagellar_pomB")
			{
			tmpstr = "pomB"
			short_protname[i] = tmpstr
			next()
		...
		if (tmpstr == "stator-force_generator_of_H+_coupled_flagellar_MotB")
			{
			tmpstr = "MotB"
			short_protname[i] = tmpstr
			next()
		...
		if (tmpstr == "Polar_sodium-driven_(_MotB)")
			{
			tmpstr = "MotB_Na_polar"
			short_protname[i] = tmpstr
			next()
	...
	return(short_protname)
	} # END classify_MotBfam_labels <- function(list_of_strings)
...
# Give a readable "head" command on a sequence DF from 
head_sdf <- function(tdf, names_to_exclude=c("Hit_def", "qseq", "hseq"))
	{
	ex='
	tdf = blastres$seqdf
	names_to_exclude=c("Hit_def", "qseq", "hseq")
	'
	colnums_to_print = colnums_wo_colnames(tdf, names_to_exclude)
	head(tdf[,colnums_to_print])
	} # END head_sdf <- function(tdf, names_to_exclude=c("Hit_def", "qseq", "hseq"))
...
# Auto-cut a taxon name to genus-species for more general searching
cut_to_genus_species <- function(taxon_names)
	{
	ex = '
	taxon_name = "Candidatus Pelagibacter ubique HTCC1062"
	cut_to_genus_species(taxon_name)
	
	taxon_name = "Caulobacter sp. K31"
	cut_to_genus_species(taxon_name)
	
	taxon_name = "Rickettsia prowazekii str. BuV67-CWPP"
	cut_to_genus_species(taxon_name)
	
	taxon_name = "Wolbachia endosymbiont of Drosophila melanogaster"
	cut_to_genus_species(taxon_name)
	...
	cut_to_genus_species(taxon_names)

	'
	
	taxon_gnsps = rep("", times=length(taxon_names))
	for (i in 1:length(taxon_names))
		{
		# Remove "Candidatus
		tmp = taxon_names[i]
		tmp1 = gsub(pattern="Candidatus", replacement="", x=tmp, ignore.case=TRUE)
		tmp1 = gdata::trim(tmp1)
	
		# Split on "sp."
		words = strsplit(tmp1, split="sp\\.")[[1]]
		keepwords = words[1]
	
		# Split on "str."
		words = strsplit(keepwords, split="str\\.")[[1]]
		keepwords = words[1]
	
		# Split on " strain"
		words = strsplit(keepwords, split=" strain")[[1]]
		keepwords = gdata::trim(words[1])
	
		# Take the remaining 1-2 words (genus, and species)
		words = strsplit(keepwords, split=" ")[[1]]
		if (length(words) >= 2)
			{
			words = words[1:2]
			} else {
		...
		taxon_gnsp = gdata::trim(paste0(words, collapse="", sep=" "))
		taxon_gnsp
		
		taxon_gnsps[i] = taxon_gnsp
		} # END for loop
	return(taxon_gnsps)
	} # END cut_to_genus_species <- function(taxon_name)


# Add taxIDs to a blastres$seqdf object
add_taxIDs <- function(seqdf, taxa_df, return_gnsp_taxID_only=FALSE)
	{
	ex='
	seqdf = blastres$seqdf
	
	taxa_df = structure(list(name = c("Agrobacterium fabrum str. C58 ", "Candidatus Pelagibacter ubique HTCC1062 ", 
"Caulobacter sp. K31 ", "Magnetococcus marinus MC-1 ", "Methylobacterium radiotolerans ATCC 27329 ", 
"Cereibacter sphaeroides ATCC 17025 ", "Rhodospirillum rubrum ATCC 11170 ", 
"Rickettsia prowazekii str. BuV67-CWPP ", "Rhizorhabdus wittichii RW1 ", 
"Wolbachia endosymbiont of Drosophila melanogaster "), taxid = c("txid176299", 
"txid335992", "txid366602", "txid156889", "txid426355", "txid349102", 
"txid269796", "txid1105096", "txid392499", "txid163164")), row.names = c(NA, 
10L), class = "data.frame")

	' # END ex
	
	taxa_names = gdata::trim(taxa_df$name)
	taxa_ids = gdata::trim(taxa_df$taxid)
	# Cut to genus-species
	taxon_gnsps = cut_to_genus_species(taxa_names)
	
	taxids = rep("", times=nrow(seqdf))
	name_taxid = rep("", times=nrow(seqdf))
	gnsp = rep("", times=nrow(seqdf))
	for (i in 1:length(taxa_names))
		{
		# Which parts of the the blastres$seqdf table match this name?
		TF = grepl(pattern=taxa_names[i], x=seqdf$Hit_def, ignore.case=TRUE)
		taxids[TF] = taxa_ids[i]
		name_taxid[TF] = taxa_names[i]
		gnsp[TF] = taxon_gnsps[i]
		}
	taxids
	...
	for (i in 1:length(taxon_gnsps))
		{
		# Which parts of the the blastres$seqdf table match this name?
		TF = grepl(pattern=taxon_gnsps[i], x=seqdf$Hit_def[not_found_TF], ignore.case=TRUE)
		taxids[not_found_TF][TF] = taxa_ids[i]
		name_taxid[not_found_TF][TF] = taxa_names[i]
		gnsp[not_found_TF][TF] = taxon_gnsps[i]
		}
	cbind(gnsp, taxids)
	
	gnsp_taxID_df = as.data.frame(cbind(taxids, name_taxid, gnsp), stringsAsFactors=FALSE)
	gnsp_taxID_df
	
	
	if (return_gnsp_taxID_only == TRUE)
		{
		return(gnsp_taxID_df)
		} else {
		numcols = ncol(seqdf)
		seqdf = cbind(seqdf[,1:2], gnsp_taxID_df, seqdf[,4:numcols])
		return(seqdf)
		}
	
	return(stop("STOP error in add_taxIDs(): shouldn't get here."))
	} # END add_taxIDs(seqdf, taxa_df)


mkdir_ifneeded <- function(tmpdir, cd=TRUE)
	{
	ex= '
	wd = "/GitHub/str2phy/ex/MitoCOGs"
	setwd(wd)
	tmpdir = "ATPase_subunit_1_Dictyostelium_discoideum"
	mkdir_ifneeded(tmpdir)
	'
	...
	tmpwd = slashslash(paste0(addslash(getwd()), tmpdir))
...
ex_Hit_def <- function()
	{
	Hit_def = c("MULTISPECIES: F0F1 ATP synthase subunit alpha [Candidatus Pelagibacter] >sp|Q4FP36.1| RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Candidatus Pelagibacter ubique HTCC1062] >gb|MDA7562580.1| F0F1 ATP synthase subunit alpha [Candidatus Pelagibacter sp.] >gb|MDA9135254.1| F0F1 ATP synthase subunit alpha [bacterium] >gb|AAZ21053.1| H+-transporting two-sector ATPase (F0F1-type ATP synthase) alpha chain [Candidatus Pelagibacter ubique HTCC1062] >gb|EAS85091.1| ATP synthase subunit A [Candidatus Pelagibacter ubique HTCC1002] >gb|MDA7443088.1| F0F1 ATP synthase subunit alpha [Candidatus Pelagibacter ubique]", 
"F0F1 ATP synthase subunit alpha [Rhodospirillum rubrum] >sp|P05036.1| RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Rhodospirillum rubrum] >sp|Q2RV20.1| RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Rhodospirillum rubrum ATCC 11170] >gb|ABC22025.1| ATP synthase F1, alpha subunit [Rhodospirillum rubrum ATCC 11170] >gb|AEO47737.1| ATP synthase F1, subunit alpha [Rhodospirillum rubrum F11] >gb|MBK5953608.1| ATP synthase subunit alpha [Rhodospirillum rubrum] >gb|QXG81680.1| F0F1 ATP synthase subunit alpha [Rhodospirillum rubrum] >emb|CAA26338.1| unnamed protein product [Rhodospirillum rubrum]", 
"MULTISPECIES: F0F1 ATP synthase subunit alpha [Methylobacterium] >sp|B1LVH1.1| RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Methylobacterium radiotolerans JCM 2831] >dbj|GAN47069.1| F0F1 ATP synthase subunit alpha [Methylobacterium sp. ME121] >gb|ACB22616.1| ATP synthase F1, alpha subunit [Methylobacterium radiotolerans JCM 2831] >gb|KTS08599.1| ATP F0F1 synthase subunit alpha [Methylobacterium radiotolerans] >gb|KTS46985.1| ATP F0F1 synthase subunit alpha [Methylobacterium radiotolerans] >gb|KZC03531.1| ATP synthase subunit alpha [Methylobacterium radiotolerans]", 
"RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Caulobacter sp. K31]", 
"F0F1 ATP synthase subunit alpha [Rhizorhabdus wittichii] >sp|A5V3X3.1| RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Rhizorhabdus wittichii RW1] >gb|ABQ66989.1| ATP synthase F1, alpha subunit [Rhizorhabdus wittichii RW1] >gb|QTH22983.1| F0F1 ATP synthase subunit alpha [Rhizorhabdus wittichii]", 
"MULTISPECIES: F0F1 ATP synthase subunit alpha [Agrobacterium] >sp|Q8UC74.1| RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Agrobacterium fabrum str. C58] >gb|AAK88345.1| ATP synthase alpha chain [Agrobacterium fabrum str. C58] >gb|AYM58314.1| ATP synthase alpha chain [Agrobacterium fabrum] >gb|AYM63369.1| ATP synthase alpha chain [Agrobacterium fabrum] >gb|EGL66164.1| F0F1 ATP synthase subunit alpha [Agrobacterium sp. ATCC 31749] >gb|MCR6723965.1| F0F1 ATP synthase subunit alpha [Agrobacterium fabrum]", 
"RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Cereibacter sphaeroides ATCC 17025]", 
"F0F1 ATP synthase subunit alpha [Magnetococcus marinus] >sp|A0LDA2.1| RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Magnetococcus marinus MC-1] >gb|ABK45945.1| ATP synthase F1 subcomplex alpha subunit [Magnetococcus marinus MC-1]", 
"MULTISPECIES: F0F1 ATP synthase subunit alpha [Wolbachia] >sp|Q73HB2.1| RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Wolbachia endosymbiont of Drosophila melanogaster] >gb|AAS14353.1| ATP synthase F1, alpha subunit [Wolbachia endosymbiont of Drosophila melanogaster] >gb|ERN55587.1| F0F1 ATP synthase subunit alpha [Wolbachia pipientis wMelPop] >gb|MCE4149154.1| F0F1 ATP synthase subunit alpha [Wolbachia endosymbiont of Drosophila melanogaster] >gb|MCE4150655.1| F0F1 ATP synthase subunit alpha [Wolbachia endosymbiont of Drosophila melanogaster] >gb|POG50004.1| F0F1 ATP synthase subunit alpha [Wolbachia sp. wMel_AMD]", 
"F0F1 ATP synthase subunit alpha [Rickettsia prowazekii] >sp|O50288.2| RecName: Full=ATP synthase subunit alpha; AltName: Full=ATP synthase F1 sector subunit alpha; AltName: Full=F-ATPase subunit alpha [Rickettsia prowazekii str. Madrid E] >gb|ADE30367.1| ATP synthase alpha chain [Rickettsia prowazekii str. Rp22] >gb|AFE50442.1| F0F1 ATP synthase subunit alpha [Rickettsia prowazekii str. Katsinyian] >gb|AFE51286.1| F0F1 ATP synthase subunit alpha [Rickettsia prowazekii str. BuV67-CWPP] >gb|AGJ01531.1| ATP synthase subunit alpha [Rickettsia prowazekii str. NMRC Madrid E] >gb|AMS12679.1| F0F1 ATP synthase subunit alpha [Rickettsia prowazekii]", 
"ATPase, FliI/YscN family [Rhizorhabdus wittichii RW1]", "MULTISPECIES: flagellar protein export ATPase FliI [Methylobacterium] >dbj|GAN49267.1| flagellum-specific ATP synthase FliI [Methylobacterium sp. ME121] >gb|ACB27519.1| flagellar protein export ATPase FliI [Methylobacterium radiotolerans JCM 2831] >gb|KTS06611.1| ATP synthase [Methylobacterium radiotolerans] >gb|KTS46461.1| ATP synthase [Methylobacterium radiotolerans] >gb|MBN6819634.1| flagellar protein export ATPase FliI [Methylobacterium organophilum]", 
"flagellar protein export ATPase FliI [Rhodospirillum rubrum] >gb|ABC21330.1| ATPase FliI/YscN [Rhodospirillum rubrum ATCC 11170] >gb|AEO47010.1| flagellum-specific ATP synthase [Rhodospirillum rubrum F11] >gb|MBK5952917.1| flagellum-specific ATP synthase [Rhodospirillum rubrum] >gb|QXG81011.1| flagellar protein export ATPase FliI [Rhodospirillum rubrum] >tpg|HAQ00692.1| flagellar protein export ATPase FliI [Rhodospirillum rubrum]", 
"MULTISPECIES: flagellar protein export ATPase FliI [Methylobacterium] >gb|ACB23703.1| ATPase, FliI/YscN family [Methylobacterium radiotolerans JCM 2831] >gb|KTS12304.1| ATP synthase [Methylobacterium radiotolerans] >gb|KTS44207.1| ATP synthase [Methylobacterium radiotolerans] >gb|RUP23071.1| MAG: flagellar protein export ATPase FliI [Methylobacterium sp.] >dbj|GEM97775.1| flagellum-specific ATP synthase [Methylobacterium radiotolerans]", 
"MULTISPECIES: F0F1 ATP synthase subunit beta [Candidatus Pelagibacter] >sp|Q4FP38.1| RecName: Full=ATP synthase subunit beta; AltName: Full=ATP synthase F1 sector subunit beta; AltName: Full=F-ATPase subunit beta [Candidatus Pelagibacter ubique HTCC1062] >gb|MDA7574997.1| F0F1 ATP synthase subunit beta [Candidatus Pelagibacter sp.] >gb|MDA9135252.1| F0F1 ATP synthase subunit beta [bacterium] >gb|MDC1245688.1| F0F1 ATP synthase subunit beta [Pelagibacteraceae bacterium] >gb|AAZ21051.1| F1-ATP Synthase Beta Chain [Candidatus Pelagibacter ubique HTCC1062] >gb|EAS85093.1| F1-ATP Synthase Beta Chain [Candidatus Pelagibacter ubique HTCC1002]", 
"F0F1 ATP synthase subunit beta [Rickettsia prowazekii] >sp|O50290.2| RecName: Full=ATP synthase subunit beta; AltName: Full=ATP synthase F1 sector subunit beta; AltName: Full=F-ATPase subunit beta [Rickettsia prowazekii str. Madrid E] >gb|AFE50440.1| F0F1 ATP synthase subunit beta [Rickettsia prowazekii str. Katsinyian] >gb|AFE51284.1| F0F1 ATP synthase subunit beta [Rickettsia prowazekii str. BuV67-CWPP] >gb|AGJ01533.1| ATP synthase subunit beta [Rickettsia prowazekii str. NMRC Madrid E] >emb|CAA15227.1| ATP SYNTHASE BETA CHAIN (atpD) [Rickettsia prowazekii str. Madrid E]", 
"hypothetical protein [Rhodospirillum rubrum] >gb|AAN75028.1| unknown [Rhodospirillum rubrum ATCC 11170] >gb|ABC23788.1| hypothetical protein Rru_A2993 [Rhodospirillum rubrum ATCC 11170] >gb|MBK5955466.1| hypothetical protein [Rhodospirillum rubrum] >gb|QXG79738.1| hypothetical protein KUL73_15435 [Rhodospirillum rubrum] >tpg|HAP99820.1| hypothetical protein [Rhodospirillum rubrum]")
	return(Hit_def)
	}

get_genbank_IDs_for_longest_seqrecs <- function(Hit_def, seq_ids)
	{
	ex='
	Hit_def = ex_Hit_def()
	# seq_ids = blastres$seqdf$accession2
	seq_ids = c("WP_006997678.1", "WP_011388979.1", "WP_012317612.1", "B0T338.1", 
"WP_011951469.1", "WP_006310773.1", "A4WUM9.1", "WP_011715001.1", 
"WP_010962741.1", "WP_004599658.1", "ABQ67655.1", "WP_012322461.1", 
"WP_011388284.1", "WP_012318691.1", "WP_006997680.1", "WP_010886376.1", 
"WP_011390741.1")
	get_genbank_IDs_for_longest_seqrecs(Hit_def)
	' # END Hit_def
	
	longest_name_seqrecs = get_longest_seqrecs(Hit_def)
	longest_name_seqrecs

	IDs_for_longest_seqrecs = gdata::trim(get_secondwords(longest_name_seqrecs, split="\\|"))
	IDs_for_longest_seqrecs
	...
	TF = is.na(IDs_for_longest_seqrecs)
	num_not_found = sum(TF)
	
	numfound = length(TF) - num_not_found
	txt = paste0("get_genbank_IDs_for_longest_seqrecs() found GenBank IDs for ", numfound, "/", length(TF), " Hit_def entries.")
	cat("\n")
	cat(txt)
	cat("\n") 
	
	return(IDs_for_longest_seqrecs)
	} # END get_IDs_for_longest_seqrecs
...
aln = read.fasta(alnfn, seqtype="AA")
